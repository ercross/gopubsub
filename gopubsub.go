...
func NewMessageBroker() *MessageBroker {
	return &MessageBroker{
		topicSubscribers: topicSubscribers{
			entries: make(map[string][]*Subscriber),
		},
		allSubscribers: allSubscribers{
...
func (mb *MessageBroker) NewSubscriber(bufferSize int, topics ...string) (*Subscriber, error) {
...
func (mb *MessageBroker) Publish(message Message) error {
	...
	subscribers, ok := mb.topicSubscribers.entries[message.Topic]
	if !ok {
